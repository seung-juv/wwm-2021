...
const MostNonsensersContainer = styled.div``;
...
const NonsensersContainer = styled.div<{ isMore: boolean }>`
  display: flex;
  flex-flow: column nowrap;
  align-items: center;
  gap: 1rem;
...
const NonsenserRank = styled.span`
  font-size: 4rem;
  letter-spacing: -0.08rem;
  color: #616067;
  font-weight: 500;
...
const NonsenserName = styled.span`
  font-size: 3rem;
  font-weight: 500;
  letter-spacing: -0.08rem;
  color: #a6a3af;
...
const MostNonsensers = (): React.ReactElement => {
  ...
  return (
    <Container>
      <SubTitle>simsimee</SubTitle>
      <Title>헛소리 가장 많이 한 사람</Title>
      <MostNonsensersContainer>
        <ProfilesContainer>
          {mostNonsensers.map(({ id, name }, index) => {
            const rank: number = (() => {
              switch (index) {
                case 1:
                  return 1;
      ...
      <NonsensersContainer isMore={isMore}>
        {nonsensers.map(({ id, name }, index) => {
          return (
            <Nonsenser key={id}>
              <NonsenserRank>{index + 4}</NonsenserRank>
              <NonsenserName>{name}</NonsenserName>
            </Nonsenser>
          );
        })}
      </NonsensersContainer>
